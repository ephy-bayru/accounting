...
export class CurrencyGridComponent implements OnInit {
  ...
  toolbarClick(args: ClickEventArgs): void {
      ...
      if (args.item.id === 'currency_add') {
        this.router.navigate(['add/currency']);
      } else if (args.item.id === 'currency_edit') {
        const selectedcur: Object = this.grid.getSelectedRecords();
        this.router.navigate(['update/currency', selectedcur[0]['id']]);
      } else if (args.item.id === 'currency_pdfexport') {
        this.grid.pdfExport();
      } else if (args.item.id === 'currency_excelexport') {
